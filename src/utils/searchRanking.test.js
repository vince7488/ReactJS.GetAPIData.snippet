...
describe('search ranking', () => {
  ...
  it('filters candidates using the fuzziness-adjusted threshold', () => {
    const candidates = [{ title: 'Pikachu' }, { title: 'Bulbasaur' }]

    expect(rankCandidates('pikchu', candidates, { fuzziness: 0, limit: 12, rankingThreshold: 0.8 }, getCandidateFields)).toEqual([])
    expect(rankCandidates('pikchu', candidates, { fuzziness: 100, limit: 12, rankingThreshold: 0.8 }, getCandidateFields)).toEqual([
      { title: 'Pikachu' },
    ])
  })
  ...
  it('ranks higher scores first and preserves provider order for ties', () => {
    ...
    expect(rankCandidates('cafe', candidates, { fuzziness: 100, limit: 2, rankingThreshold: 0.8 }, getCandidateFields)).toEqual([
      candidates[0],
      candidates[1],
    ])
  })
  ...
  it('matches query tokens across multiple provider candidate fields', () => {
    ...
    expect(
      rankCandidates('hobbit tolkien', candidates, { fuzziness: 0, limit: 12, rankingThreshold: 0.8 }, (candidate) => [
        candidate.title,
        candidate.author,
      ]),
    ).toEqual(candidates)
